...
void setInputBits   (std::vector<OFDM::Modulator> &modulatorVector, const std::array<int,16> &inputBits, const int bitsPerSymbol);
...
void calculateDFT (std::vector<OFDM::Demodulator> &demodulatorVector, const std::vector<double> &DFTinput, 
    std::vector<std::vector<std::complex<double>>> &DFToutput, const int N);
void sumOfSubcarriers(std::vector<OFDM::Demodulator> &demodulatorVector, const std::vector<std::vector<std::complex<double>>> &DFToutput, std::vector<std::complex<double>> &DFTsum, const int N);
void extractZ (std::vector<OFDM::Demodulator> &demodulatorVector, 
    const std::vector<std::vector<std::complex<double>>> &DFToutput, std::vector<std::complex<double>> &DFTsum, const std::vector<double> fAnalysis, const std::vector<double> &frequencies, const double ts, const int N);
